...
public class ChatListAdapter extends CursorAdapter {
    ...
    public void mergeLastPageCursor(Cursor pageCursor) {
        CustomLog.i(TAG, "mergeLastPageCursor()");
    ...
    public int getItemViewType(int position) {
        CustomLog.i(TAG, "getItemViewType()" + " | position = " + position);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        CustomLog.i(TAG, "getView()");

        final NoticesBean bean = NoticesTable.pureChatCursor(getItem(position),
            noticeType);
        CustomLog.i(TAG, "getView() | position = " + position + " | type = " + bean.getType()
            + bean.getBody());
    ...
    private View createViewByNotice(NoticesBean bean) {
        switch (bean.getType()) {
            ...
            case FileTaskManager.NOTICE_TYPE_ATTACHMENT_FILE:// 附件消息 （word, pdf）
                return isSendNotice(bean.getSender()) ? layoutInflater.inflate(
    ...
    public void stopCurAuPlaying() {
        ...
        notifyDataSetChanged(); // NOTE: 目的是解决语音消息第一次播放时，动画无法停止问题。暂时解决，还需仔细排查
    ...
    private void detachUploadProgressView(TextView view, String id) {
        FileTaskProgressManager progressListener = null;
        Object tagObj = view.getTag();
        if (tagObj != null) {
            progressListener = (FileTaskProgressManager) tagObj;
    ...
    private void attachUploadProgressView(TextView view, String id) {
        ...
        if (tagObj != null) {
            ...
            progressListener = new FileTaskProgressManager(view, id);
     ...
     * 文件上传下载进度管理
     */
    private class FileTaskProgressManager extends ChangeUIInterface {
        ...
        public void setAttachedView(TextView view) {
            CustomLog.i(TAG, "setAttachedView()");

            viewReference = new WeakReference<TextView>(view);
        }


        public FileTaskProgressManager(TextView view, String _id) {
            viewReference = new WeakReference<TextView>(view);
            id = _id;
        }
        ...
        public void onStart(FileTaskBean bean) {
            // 开始文件任务
            CustomLog.i(TAG, "开始文件任务 onStart：" + bean.getUuid());
        }


        public void onProcessing(FileTaskBean bean, long current, long total) {
            // 文件任务进度
            CustomLog.d(TAG, "onProcessing()");
            ...
            if (!changUIProgressTaskIds.contains(bean.getUuid())) {
                // 已不需要更新界面
                CustomLog.d(TAG, "onProcessing:已不需要更新界面");
                return;
            }
    ...
    private class FileProgressListener extends ChangeUIInterface {
        ...
        public void onStart(FileTaskBean bean) {
            // 开始文件任务
            CustomLog.d(TAG, "开始文件任务 onStart：" + bean.getUuid());
        }
        ...
        public void onSuccess(FileTaskBean bean, String result) {
            // 文件任务成功完成
            CustomLog.d(TAG, "onSuccess:" + bean.getUuid() + ":" + result);
        }
    ...
    private Object getCursorDataByCol(Cursor cursor, String column, int type) {
        ...
        if (cursor != null && !cursor.isClosed()) {
            try {
                ...
                CustomLog.e(TAG, "getCursorDataByCol Exception" + e.toString());
            }
        }
        return 0;
    }
                    ...
                    menuDlg.addButtonFirst(new MenuClickedListener() {
                        ...
                        public void onMenuClicked() {
                            if (callbackIf != null) {
                                callbackIf.onMsgForward(bean.getId(),
                                    bean.getSender(), bean.getType(),
                                    bean.getStatus(), null, holder.noticeTxt.getText().toString(),
